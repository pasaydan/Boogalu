...
export default function UsersInfo() {
  ...
  const [shoulGenericModalHasAction, toggleGenericModalAction] =
    useState(false);
  ...
  function videoDeleteConfirmationResponse(
    action,
    confirmed,
    userKey,
    videoKey,
  ...
  function searchUser(event) {
    event.stopPropagation();
    const userInputData = userSearchInputRef.current
      ? userSearchInputRef.current.value
      : "";
    if (userInputData) {
  ...
  function openOfferModalBox(event, user) {
    ...
    if (isUserOfferAvailed) {
      ...
      setInfoModalMessage(
        `${user?.email} already availed this offer as a part of event registration!`
      );
      ...
      setInfoModalMessage(
        `Are you sure you want to give this offer to ${user?.email}`
      );
      setInfoModalStatus("info");
  ...
  function confirmedOfferYes(action) {
    if (action) {
      let offerSub = subscriptionsList.filter(
        (subData) => subData.planType === "startup"
      );
      ...
      if (
        updatedUserData &&
        updatedUserData?.subscribed &&
        updatedUserData?.planType === (offerSub[0]?.planType || "startup") &&
        updatedUserData?.subscriptions &&
        updatedUserData?.subscriptions?.length
      ) {
        updatedUserData?.subscriptions &&
          updatedUserData?.subscriptions.length &&
          updatedUserData?.subscriptions.forEach((subData) => {
            if (
              subData?.planType === offerSub[0]?.planType &&
              !subData.isExpired
            ) {
              subData.validity += 2;
            }
        ...
        updateUser(updatedUserData.key, updatedUserData).subscribe(
          (response) => {
            ...
            if (response?.updated) {
              ...
              setInfoModalMessage(
                `2 months free subscription offer applied to ${updatedUserData.email}`
              );
    ...
    <div className="adminPanelSection">
                ...
                {userVideoDataList.map((item, index) => {
                  return (
                    <div className="videoDetails" key={`userVideo-id-${index}`}>
                      ...
                      <p className="subText">{item.uploadedTime}</p>
                      <p
                        className="deleteVideoIcon"
      ...
      >
        {isAdminLoggedIn || checkAdminLogIn ? (
          <div
            className="logOutIconWrap filterIconWrap searchIconWrap"
            title="search user by email/phone"
            onClick={(e) => toggleSearchInput(e, isUserSearchOpen)}
          >
            ...
            {isUserSearchOpen ? <FaTimes /> : <FaSearch />}
          </div>
        ) : (
          ""
        )}
        {isAdminLoggedIn || checkAdminLogIn ? (
          <p
            className="logOutIconWrap filterIconWrap"
            title="Open Filter"
              ...
              {userListData && userListData.length ? (
                          ...
                          <tr key={`user-item-${index}`}>
                            ...
                            <td>
                              <div className="actionBlock">
                                <p
                                  className="viewUserIcon"
                                  title="View users videos"
                                  onClick={(e) =>
                                    fetchUsersVideoDetails(e, item.key)
                                  }
                                >
                                  <FiEye />
                                </p>
                                ...
                                <p
                                  ...
                                  onClick={(e) => openOfferModalBox(e, item)}
                                >
                                  <FiGift />
                                </p>
                              </div>
                ...
                <tbody className="filterTbody">
                  <tr>
                    <td>
                      <p>
                        No user available with the selected filter, try applying
                        different filters!
                      </p>
                    </td>
                  </tr>
                ...
                <tbody className="emptyTbody">
                  <tr>
                    <td>
                      <p>
                        It seems Boogaluu doesn't have any users at this moment!
                      </p>
                    </td>
                  </tr>
                </tbody>
          ...
          <div className="innerBox">
            ...
            <div className="filterActionWrap">
              ...
              <button
                ...
                onClick={(e) => applyUserFilter(e, false)}
              >
                Apply
              </button>
            </div>
