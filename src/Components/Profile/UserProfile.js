...
function Profile() {
  ...
  useEffect(() => {
    ...
    toggleLoading(true);
    getCompetitionByUserId(loggedInUser.key).subscribe((list) => {
      toggleLoading(false);
  ...
  const handleComments = (commentString) => {
    ...
    videoObj.comments.map((commentObj) => {
      ...
      <div className="profile-details-wrap clearfix">
        ...
        <div className="profile-details clearfix">
          ...
          <div className="bio-wrap">
            <div className="fullname">{loggedInUser.name}</div>
            {loggedInUser.bio ? (
